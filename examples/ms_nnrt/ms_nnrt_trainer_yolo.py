...
class Trainer(base.Trainer):
    ...
    def randomize(self, bit_array: np.ndarray, targets: np.ndarray, epsilon):
        ...
        img = unary_encoding_1b.symmetric_unary_encoding(bit_array, 100)
    ...
    def convert(self, size, box):
        ...
        x1 = max(round(x1/640 * size[0]), 0)
        x2 = min(round(x2/640 * size[0]), size[0])
        y1 = max(round(y1/640 * size[1]), 0)
        y2 = min(round(y2/640 * size[1]), size[1])
